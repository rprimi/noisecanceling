...
# subj id
  data <- data %>% dplyr::select(unique(item_dic$coditem))

# Dictionnaire expects: "coditem", "scale", "pole", "seman_pairs", "item_text"
...
# Acquiescence items
  coditem_acq <- unique(item_dic[!is.na(item_dic$seman_pairs), ]$coditem)

  # Acquiescence index and sds global
  acq_index <- apply(data[ ,coditem_acq], 1, function(x) {mean(x, na.rm=TRUE)} )
  ws_sd <- apply(data[ , coditem_acq], 1, function(x) {sd(x, na.rm=TRUE)} )
  ws_sd <- ifelse(ws_sd == 0, NA, ws_sd)

  # Acquiescence index by scale
  acq_index_by_scale <- lapply(
     split(item_dic_acq,
           f = factor(item_dic_acq$scale,
             levels = unique(item_dic_acq$scale))
       ),
      function(x){
          apply(data[, x$coditem], 1,function(y){ mean(y, na.rm = TRUE)})
      }
  )

  ws_sd_by_scale <- lapply(
    split(item_dic_acq,
      f = factor(item_dic_acq$scale,
        levels = unique(item_dic_acq$scale))
     ),
      function(x){
          apply(data[, x$coditem], 1,function(y){ sd(y, na.rm = TRUE)})
      }
  )

  names(acq_index_by_scale) <- paste("acq", names(acq_index_by_scale), sep="_")
  names(ws_sd_by_scale) <- paste("ws_sd", names(ws_sd_by_scale), sep="_")

  acq_index_by_scale <-as.data.frame(acq_index_by_scale)
  ws_sd_by_scale <-as.data.frame(ws_sd_by_scale)
  ...
  # Center responses
  data_c_rec <-  as.data.frame(
      apply(data[, v_tst], 2, function(x) {(x - acq_index)}
      )
  )
  ...
  results <- list(
      data= data,
      data_acq_recoded = data_c_rec,
      acq_index = cbind( acq_index, ws_sd,
          acq_index_by_scale, ws_sd_by_scale),
      item_dic = item_dic,
...
# original
  min = apply(obj$data[ , rownames(keys)], 2,function(x){ min(x, na.rm = TRUE)})
  max = apply(obj$data[ , rownames(keys)], 2,function(x){ max(x, na.rm = TRUE)})
...
# Recoded
  min = apply(obj$data_acq_recoded[, rownames(keys)], 2,function(x){ min(x, na.rm = TRUE)})
  max = apply(obj$data_acq_recoded[, rownames(keys)], 2,function(x){ max(x, na.rm = TRUE)})
...
save_item_psicom <- function(obj, filename) {
 ...
 # por causa de um pau no psych
  ...
  scale_stats_ori <- rbind(
    ...
    t(describe(obj$psicom_ori$scores))
  )
  ...
  scale_stats_rec <- rbind(
    ...
    t(describe(obj$psicom_recoded$scores))
...
dic2keys <- function(item_dic, reversed = TRUE ) {
 ...
 return(as.matrix(keys[ , unique(item_dic$scale)]))
}
