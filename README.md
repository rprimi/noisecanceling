...
![](man/figures/what_is_acq.jpg) ![](man/figures/f2.jpg)
...
`recode_for_acq()` estimates a per-person acquiescence index from the
balanced (paired) items and centers every response on it.
`find_psychometrics()` then compares the psychometrics of the original
and the acquiescence-corrected scores.
...
`item_histograms()` and `describe_likert()` for inspecting item
distributions, and `save_item_psicom()` / `save_loadings()` for
